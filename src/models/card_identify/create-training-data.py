...
try:
    ...
    # paths
    file_stream.write("path: C:\\Users\\dmiller3905\\Documents\\clash_royale_ai\\src\\models\\card-identify\\dataset"+"\n")
...
sprites = {}
training_by_troop = False # change this to false if we want to train on every troop
troops_to_train = [ "knight", "archer", "minion", "arrows", "fireball", "giant", "mini-pekka", "musketeer", "goblin", "goblin-cage", "goblin-brawler", "goblin-hut", "spear-goblin", "cannon", "barbarian"] # if we want to train specific cards, specify what cards here
for troop_id, troop_name in enumerate(troop_names):
    sprites[troop_id] = []
    ...
    for troop_image_path in troop_images_path.iterdir():
        ...
        if troop_image_path.is_file():
            ...
            sprite = sprite.resize((50, 62)).convert("RGBA")
...
def populate_training_data(training_image_count=10):
    ...
    for idx in range(training_image_count):
        ...
        training_image, training_annotation_arr = create_training_image(bg, sprites, sprites_to_place=5)
